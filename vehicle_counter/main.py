import cv2
from vehicle_counter.consts import *
...
def logical_frame():
    ...
    try:
        ...
        opening = cv2.morphologyEx(threshold, cv2.MORPH_OPEN, kernel, iterations=opening_iterations)
        dilatation = cv2.dilate(opening, kernel, iterations=dilatation_iterations)
        closing = cv2.morphologyEx(dilatation, cv2.MORPH_CLOSE, kernel, iterations=closing_iterations)  # Máscara final
...
if __name__ == '__main__':
    cap = cv2.VideoCapture('../resources/gd_vision.MOV')
